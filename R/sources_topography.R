...
sources_topography <- function(name,formats,time_resolutions, ...) {
    ...
    if (is.null(name) || any(name %in% tolower(c("Smith and Sandwell bathymetry", "global_topo_1min")))) {
        out <- rbind(out,
                     bb_source(
                         ...
                         doc_url = "https://topex.ucsd.edu/marine_topo/",
                         ...
                         source_url = "https://topex.ucsd.edu/pub/global_topo_1min/",
                         license = "See https://topex.ucsd.edu/pub/global_topo_1min/README_PERMISSIONS.txt",
    ...
    if (is.null(name) || any(name %in% tolower(c("GEBCO 2024 bathymetry", "GEBCO_2024")))) {
        out <- rbind(out,
                     bb_source(
                         name = "GEBCO 2024 bathymetry",
                         id = "GEBCO_2024",
                         description = "The GEBCO_2024 Grid is a global terrain model for ocean and land, providing elevation data, in meters, on a 15 arc-second interval grid of 43200 rows x 86400 columns, giving 3,732,480,000 data points. The data values are pixel-centre registered i.e. they refer to elevations, in meters, at the centre of grid cells.",
                         doc_url = "https://www.gebco.net/data_and_products/gridded_bathymetry_data/gebco_2024/",
                         ...
                         method = list("bb_handler_rget", force_local_filename = "gebco_2024_netcdf.zip"),
                         comment = "bowerbird cannot currently unzip the GEBCO 2024 files. You will need to unzip manually with a utility that can handle large files (e.g. 7z on Linux systems)",
                         collection_size = 4.0,
                         data_group = "Topography")
                     )
    }
    ...
    if (is.null(name) || any(name %in% tolower(c("Revision of the Kerguelen Plateau bathymetric grid", "gcat_71552_rev")))) {
        out <- rbind(out,
                     bb_source(
                         ...
                         comment = "This product has been superseded by \"AusBathyTopo (Kerguelen Plateau) 100m 2022\" - you are advised to use that for new projects. \"Revision of the Kerguelen Plateau bathymetric grid\" is a revision of the \"Kerguelen Plateau bathymetric grid 2010\" data source, which is no longer available",
    ...
    if (is.null(name) || any(name %in% tolower(c("AusBathyTopo (Kerguelen Plateau) 100m 2022", "10.26186/147703")))) {
        out <- rbind(out,
                     bb_source(
                         name = "AusBathyTopo (Kerguelen Plateau) 100m 2022",
                         id = "10.26186/147703",
                         description = "This dataset contains bathymetry (depth) products from the compilation of all available source bathymetry data within the Kerguelen Plateau into a 100 m-resolution Digital Elevation Model (DEM).",
                         comment = "Supersedes \"Revision of the Kerguelen Plateau bathymetric grid\"",
                         doc_url = "https://doi.org/10.26186/147703",
                         citation = "Beaman RJ (2023) AusBathyTopo (Kerguelen Plateau) 100m 2022 - A High-resolution Depth Model (20220004C). Geoscience Australia, Canberra. 10.26186/147703",
                         source_url = "https://files.ausseabed.gov.au/survey/Kerguelen%20Plateau%202022%20100m.zip",
    ...
    if (is.null(name) || any(name %in% tolower(c("Radarsat Antarctic digital elevation model V2", "nsidc0082")))) {
        out <- rbind(out,
                     bb_source(
                         ...
                         license = "Please cite",
                         source_url = "https://daacdata.apps.nsidc.org/pub/DATASETS/nsidc0082_radarsat_dem_v02",
                         authentication_note = "Requires Earthdata login, see https://urs.earthdata.nasa.gov/",
                         method = list("bb_handler_earthdata", level = 3, accept_download_extra = "\\.hdr$", reject_download = "\\.txt\\.gz$", relative = TRUE, allow_unrestricted_auth = TRUE),
                         postprocess = list("bb_gunzip"),
                         collection_size = 5.3,
                         data_group = "Topography"))
    }
