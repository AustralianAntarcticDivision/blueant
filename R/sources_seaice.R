...
sources_seaice <- function(name, formats, time_resolutions, ...) {
    ...
    if (is.null(name) || any(name %in% tolower(c("NOAA/NSIDC Climate Data Record of Passive Microwave Sea Ice Concentration, Version 4", "10.7265/efmz-2t65")))) {
        out <- rbind(out,
                     bb_source(
    ...
    if (is.null(name) || any(name %in% tolower(c("Near-Real-Time NOAA/NSIDC Climate Data Record of Passive Microwave Sea Ice Concentration, Version 2", "10.7265/tgam-yv28")))) {
        out <- rbind(out,
                     bb_source(
