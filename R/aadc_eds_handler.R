...
aadc_eds_get <- function(cfrow,verbose=FALSE,local_dir_only=FALSE) {
    ...
    if (any(slidx)) {
        ...
        cfrow$source_url[slidx] <- paste0(cfrow$source_url[slidx],"/")
    }
    ...
    if (grepl("/download",cfrow$source_url)) {
        this_file_id <- str_match(cfrow$source_url,"/eds/(\\d+)/download$")[2]
        if (is.na(this_file_id)) stop("could not determine AADC EDS file_id")
        trailing_path <- file.path("data.aad.gov.au","eds",this_file_id)
    } else {
        this_file_id <- str_match(cfrow$source_url,"/eds/file/(\\d+)/?$")[2]
        if (is.na(this_file_id)) stop("could not determine AADC EDS file_id")
        trailing_path <- file.path("data.aad.gov.au","eds","file",this_file_id)
        if (!grepl("/$",cfrow$source_url))
            cfrow$source_url <- paste0(cfrow$source_url,"/") ## this form needs trgailing /
    }
    if (local_dir_only) return(file.path(cfrow$local_file_root,trailing_path))
    if (!file.exists(file.path(cfrow$local_file_root,trailing_path))) {
        dir.create(file.path(cfrow$local_file_root,trailing_path),recursive=TRUE)
    }
