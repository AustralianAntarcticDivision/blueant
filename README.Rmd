...
[![CRAN_Status_Badge](http://www.r-pkg.org/badges/version/blueant)](http://cran.r-project.org/web/packages/blueant)
<!-- ![downloads](http://cranlogs.r-pkg.org/badges/grand-total/blueant) -->
...
```{r include = FALSE}
## code not shown in the README: (1) use load_all() instead of library(blueant) for convenience, (2) use fixed "temporary" dir so we don't re-download the zip multiple times, and (3) hide the progress bar
devtools::load_all()
my_data_dir <- "c:/tmp/data" ##tempdir()#file.path(dirname(tempdir()), "RtmpjAqcVC")
if (!dir.exists(my_data_dir)) dir.create(my_data_dir)
file.remove(dir(my_data_dir, pattern = "gvdem.*\\.nc$", full.names = TRUE, recursive = TRUE))
